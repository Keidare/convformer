...
class ConvBlockBN(nn.Module):
    def __init__(self,
        ...
        if reparameterize:
            ...
            dw_conv = Conv2d_BN(dim, dim, kernel, stride, padding=kernel // 2, groups=dim)  # depthwise conv
...
class RepCPEBN(nn.Module):
    def __init__(self, dim, kernel=3, **kargs):
        super().__init__()
        self.cpe = Residual(
            Conv2d_BN(dim, dim, kernel, 1, padding=kernel//2, groups=dim)  # depthwise conv with LN
        )
...
class EdgeResidualBN(nn.Module):
    """ FusedIB in MobileNetV4-like architectures, adapted to use LayerNorm instead of BatchNorm.
    """
    def __init__(self,
                 in_chs: int,
        ...
        self.conv_exp_ln1 = Conv2d_BN(in_chs, mid_chs, exp_kernel_size, stride, padding=exp_kernel_size//2)
        self.act = act_layer()
        self.conv_pwl_ln2 = Conv2d_BN(mid_chs, out_chs, 1)
