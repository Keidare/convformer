...
class ConvFormer(nn.Module):
    def __init__(
        ...
        stem = nn.Sequential(
            Conv2d_LN(1, dims[0] // 2, stem_kernel, 2, padding=stem_kernel // 2),
            act_layer(),
        ...
        stage = nn.Sequential(
            *[ConvBlock(dim=dims[2], drop_path=dp_rates[cur + j], layer_scale_init_value=layer_scale_init_value) for j in range(7)],
            AttnBlock(dim=dims[2], sr_ratio=2, head=5, dpr=dp_rates[cur + 7]),
            # *[ConvBlock(dim=dims[2], drop_path=dp_rates[cur + j + 8], layer_scale_init_value=layer_scale_init_value) for j in range(2)],
            # AttnBlock(dim=dims[2], sr_ratio=2, head=4, dpr=dp_rates[cur + 8]),
            AttnBlock(dim=dims[2], sr_ratio=2, head=5, dpr=dp_rates[cur + 8]),
        )
        self.stages.append(stage)
        cur += 9
...
class ConvFormerV2(nn.Module):
    def __init__(
        ...
        stage = nn.Sequential(
            ...
            *[AttnBlock(dim=dims[2], sr_ratio=2, head=4, dpr=dp_rates[cur + j + 9]) for j in range(3)]
        )
        self.stages.append(stage)
        cur += 12
    ...
    def forward(self, x):
        ...
        for i in range(4):
            ...
            features.append(x)  # Store feature maps from each stage
        x = self.norm(x.mean([-2, -1]))
        out = torch.sigmoid(self.CLhead(x))
        return out  # Return multi-scale feature maps
